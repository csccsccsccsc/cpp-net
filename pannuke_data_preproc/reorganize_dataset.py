...
for ifold in [1,2,3]:
    ...
    image_dir = PannukeRoot + '/fold_'+str(ifold)+'/images/fold'+str(ifold)+'/images.npy'
    masks_dir = PannukeRoot + '/fold_'+str(ifold)+'/masks/fold'+str(ifold)+'/masks.npy'
    imgs = np.load(image_dir).astype(np.uint8)
    msks = np.load(masks_dir).astype(np.uint16)

    img_filefold_tosave = os.path.join(TgtSaveRoot, 'reorganized_dataset', 'fold_'+str(ifold), 'images')
    msk_filefold_tosave = os.path.join(TgtSaveRoot, 'reorganized_dataset', 'fold_'+str(ifold), 'masks')
