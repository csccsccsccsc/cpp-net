...
def predict_each_image(
    model_dist, img, axis_norm=[0,1], center_prob_thres=0.4, seg_prob_thres=0.5, n_rays=32, FPP=True, sin_angles=None, cos_angles=None, dist_cmp='cuda'):
    ...
    with torch.no_grad():
        ...
        for idx, img_target in enumerate(zip(X,Y)):
            ...
            star_label = predict_each_image(
                ...
                center_prob_thres=center_prob_thres, seg_prob_thres=seg_prob_thres, n_rays=n_rayus
            )
        ...
        for itau, iou in enumerate(ap_ious):
            print(iou, stats[ap_ious.index(tau)].accuracy)
            avg += stats[ap_ious.index(tau)].accuracy
        avg /= len(ap_ious)
...
DATASET_PATH_IMAGE = 'PATH TO YOUR DATASET/test/images/*.tif'
DATASET_PATH_LABEL = 'PATH TO YOUR DATASET/test/masks/*.tif'


MODEL_WEIGHT_PATH = 'MODEL WEIGHT PATH'
n_rays = 32
nc_in = 1
