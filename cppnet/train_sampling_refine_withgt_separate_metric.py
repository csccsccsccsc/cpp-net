...
class Trainer():
    ...
    def pretrain(self, model, DataSet, optimizer, epoch): #eval is not correct in the method
        _loss=0
        _correct=0
        model.train()
        kwargs ={}
        ...
        for iepoch in range(epoch):
            for batch_idx, data in enumerate(DataSet):
                if len(data) == 3:
                    ...
                    if self.USE_CUDA:
                        ...
                        model.cuda()
                optimizer.zero_grad()
                ## While training, using ground truth distances for sampling
                prediction =  model(inputs, distances)
                
                distances= distances.squeeze(1)
                total_loss, total_metric = self.loss_ce(prediction, distances,**kwargs)
                total_loss.backward()
                optimizer.step()
                _loss += total_metric.item()
                if self.metric is not None:
                    prediction_final = [prediction[2], prediction[3]]
                    _correct += self.metric(prediction_final,distances)

            _loss_average=_loss/len(DataSet.dataset)/epoch
            if self.metric is not None:
            ...
            print('Current Erosion List: ')
            e_list = model.erosion_factor_list
            if isinstance(e_list, nn.Parameter):
                print(e_list.data.cpu().numpy())
            else:
                print(e_list)
    ...
    def train_scratch(self,model,DataSet,optimizer,epoch): #eval is not correct in the method
        ...
        for batch_idx, data in enumerate(DataSet):
            ...
            if self.tb_logger is not None:
                ...
                self.tb_logger.log_scalar(tag='train_Loss', value = total_metric.item(), step=step)
                ...
                if step % log_image_interval == 0:
                    ...
                    if isinstance(prediction, list):
                        prediction = prediction[3] #take object probabilities
    ...
    def evaluate(self,model,DataSet,epoch=None):
        ...
        with torch.no_grad():
            for batch_idx, data in enumerate(DataSet):
                ...
                if self.metric is not None:
                    prediction_final = [prediction[2], prediction[3]]
                    _correct += self.metric(prediction_final, target)
            _loss_average =_loss/len(DataSet.dataset)
            if self.metric is not None:
                _acc=_correct/float(batch_idx+1)# Average over all batches
                if self.verborrea:
